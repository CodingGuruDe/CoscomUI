...
import UploadIcon from '@coscom/coscom-ui/icons/upload';
import Message from '@coscom/coscom-ui/message';
import ProgressBar from '@coscom/coscom-ui/progressbar';
...
export default {
    ...
    methods: {
        ...
        formatSize(bytes) {
            const k = 1024;
            const dm = 3;
            const sizes = this.$coscom.config.locale?.fileSizeTypes || ['B', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    ...
    computed: {
        ...
        chooseButtonLabel() {
            return this.chooseLabel || this.$coscom.config.locale.choose;
        },
        uploadButtonLabel() {
            return this.uploadLabel || this.$coscom.config.locale.upload;
        },
        cancelButtonLabel() {
            return this.cancelLabel || this.$coscom.config.locale.cancel;
        },
        completedLabel() {
            return this.$coscom.config.locale.completed;
        },
        pendingLabel() {
            return this.$coscom.config.locale.pending;
        }
    },
    components: {
