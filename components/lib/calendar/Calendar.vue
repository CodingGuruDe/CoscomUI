<template>
    <span ref="container" :id="d_id" :class="cx('root')" :style="sx('root')" v-bind="ptmi('root')">
        ...
        <CalendarButton
            ...
            @click="onButtonClick"
            type="button"
            :aria-label="$coscom.config.locale.chooseDate"
            aria-haspopup="dialog"
            :aria-expanded="overlayVisible"
            :aria-controls="panelId"
            :unstyled="unstyled"
                            ...
                            <div v-for="(month, groupIndex) of months" :key="month.month + month.year" :class="cx('group')" v-bind="ptm('group')">
                                <div :class="cx('header')" v-bind="ptm('header')">
                                    ...
                                    <button
                                        ...
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $coscom.config.locale.prevDecade : currentView === 'month' ? $coscom.config.locale.prevYear : $coscom.config.locale.prevMonth"
                                    ...
                                    <div :class="cx('title')" v-bind="ptm('title')">
                                        <template v-if="$coscom.config.locale.showMonthAfterYear">
                                            <button
                                                ...
                                                :aria-label="$coscom.config.locale.chooseYear"
                                            ...
                                            <button
                                                ...
                                                :class="cx('monthTitle')"
                                                :disabled="switchViewButtonDisabled"
                                                :aria-label="$coscom.config.locale.chooseMonth"
                                    ...
                                    <button
                                        ...
                                        @keydown="onContainerButtonKeydown"
                                        :disabled="disabled"
                                        :aria-label="currentView === 'year' ? $coscom.config.locale.nextDecade : currentView === 'month' ? $coscom.config.locale.nextYear : $coscom.config.locale.nextMonth"
                    ...
                    <div v-if="(showTime || timeOnly) && currentView === 'date'" :class="cx('timePicker')" v-bind="ptm('timePicker')">
                        <div :class="cx('hourPicker')" v-bind="ptm('hourPicker')" data-pc-group-section="timepickerContainer">
                            <button
                                ...
                                :aria-label="$coscom.config.locale.nextHour"
                            ...
                            <button
                                ...
                                :aria-label="$coscom.config.locale.prevHour"
                                @mousedown="onTimePickerElementMouseDown($event, 0, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                                @mouseleave="onTimePickerElementMouseLeave()"
                        ...
                        <div :class="cx('minutePicker')" v-bind="ptm('minutePicker')" data-pc-group-section="timepickerContainer">
                            <button
                                v-ripple
                                :class="cx('incrementButton')"
                                :aria-label="$coscom.config.locale.nextMinute"
                            ...
                            <button
                                v-ripple
                                :class="cx('decrementButton')"
                                :aria-label="$coscom.config.locale.prevMinute"
                                @mousedown="onTimePickerElementMouseDown($event, 1, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                                :disabled="disabled"
                        ...
                        <div v-if="showSeconds" :class="cx('secondPicker')" v-bind="ptm('secondPicker')" data-pc-group-section="timepickerContainer">
                            <button
                                v-ripple
                                :class="cx('incrementButton')"
                                :aria-label="$coscom.config.locale.nextSecond"
                            ...
                            <button
                                v-ripple
                                :class="cx('decrementButton')"
                                :aria-label="$coscom.config.locale.prevSecond"
                                @mousedown="onTimePickerElementMouseDown($event, 2, -1)"
                                @mouseup="onTimePickerElementMouseUp($event)"
                                @keydown="onContainerButtonKeydown"
                                :disabled="disabled"
                        ...
                        <div v-if="hourFormat == '12'" :class="cx('ampmPicker')" v-bind="ptm('ampmPicker')">
                            <button
                                v-ripple
                                :class="cx('incrementButton')"
                                :aria-label="$coscom.config.locale.am"
                            ...
                            <span v-bind="ptm('ampm')" data-pc-group-section="timepickerlabel">{{ pm ? $coscom.config.locale.pm : $coscom.config.locale.am }}</span>
                            <button
                                v-ripple
                                :class="cx('decrementButton')"
                                :aria-label="$coscom.config.locale.pm"
                                @click="toggleAMPM($event)"
                                @keydown="onContainerButtonKeydown"
                                type="button"
                                :disabled="disabled"
...
import CalendarIcon from '@coscom/coscom-ui/icons/calendar';
import ChevronDownIcon from '@coscom/coscom-ui/icons/chevrondown';
import ChevronLeftIcon from '@coscom/coscom-ui/icons/chevronleft';
import ChevronRightIcon from '@coscom/coscom-ui/icons/chevronright';
import ChevronUpIcon from '@coscom/coscom-ui/icons/chevronup';
import OverlayEventBus from '@coscom/coscom-ui/overlayeventbus';
import Portal from '@coscom/coscom-ui/portal';
import Ripple from '@coscom/coscom-ui/ripple';
import { ConnectedOverlayScrollHandler, DomHandler, UniqueComponentId, ZIndexUtils } from '@coscom/coscom-ui/utils';
...
export default {
    ...
    methods: {
        ...
        onOverlayEnter(el) {
            ...
            if (this.autoZIndex) {
                if (this.touchUI) ZIndexUtils.set('modal', el, this.baseZIndex || this.$coscom.config.zIndex.modal);
                else ZIndexUtils.set('overlay', el, this.baseZIndex || this.$coscom.config.zIndex.overlay);
            }
                    ...
                    if (literal) {
                        ...
                        switch (format.charAt(iFormat)) {
                            case 'd':
                                ...
                                break;
                            case 'D':
                                output += formatName('D', date.getDay(), this.$coscom.config.locale.dayNamesShort, this.$coscom.config.locale.dayNames);
                                break;
                            case 'o':
                            ...
                            case 'm':
                                ...
                                break;
                            case 'M':
                                output += formatName('M', date.getMonth(), this.$coscom.config.locale.monthNamesShort, this.$coscom.config.locale.monthNames);
                                break;
                            case 'y':
        ...
        formatTime(date) {
            ...
            if (this.hourFormat === '12') {
                output += date.getHours() > 11 ? ` ${this.$coscom.config.locale.pm}` : ` ${this.$coscom.config.locale.am}`;
            }
        ...
        populateTime(value, timeString, ampm) {
            ...
            this.pm = ampm === this.$coscom.config.locale.pm || ampm === this.$coscom.config.locale.pm.toLowerCase();
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        case 'd':
                            ...
                            break;
                        case 'D':
                            getName('D', this.$coscom.config.locale.dayNamesShort, this.$coscom.config.locale.dayNames);
                            break;
                        case 'o':
                            doy = getNumber('o');
                            break;
                        case 'm':
                            ...
                            break;
                        case 'M':
                            month = getName('M', this.$coscom.config.locale.monthNamesShort, this.$coscom.config.locale.monthNames);
                            break;
                        case 'y':
                            year = getNumber('y');
                            break;
        ...
        getMonthName(index) {
            return this.$coscom.config.locale.monthNames[index];
        },
        getYear(month) {
    ...
    computed: {
        ...
        weekDays() {
            let weekDays = [];
            let dayIndex = this.$coscom.config.locale.firstDayOfWeek;

            for (let i = 0; i < 7; i++) {
                weekDays.push(this.$coscom.config.locale.dayNamesMin[dayIndex]);
        ...
        datePattern() {
            return this.dateFormat || this.$coscom.config.locale.dateFormat;
        },
        yearOptions() {
            if (this.yearRange) {
                let $vm = this;
        ...
        monthPickerValues() {
            ...
            for (let i = 0; i <= 11; i++) {
                monthPickerValues.push({ value: this.$coscom.config.locale.monthNamesShort[i], selectable: isSelectableMonth(i) });
            }
        ...
        todayLabel() {
            return this.$coscom.config.locale.today;
        },
        clearLabel() {
            return this.$coscom.config.locale.clear;
        },
        weekHeaderLabel() {
            return this.$coscom.config.locale.weekHeader;
        },
        monthNames() {
            return this.$coscom.config.locale.monthNames;
        },
        attributeSelector() {
