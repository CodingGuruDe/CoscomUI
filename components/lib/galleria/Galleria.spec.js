...
describe('Gallleria.vue', () => {
    ...
    it('should exist', () => {
        expect(wrapper.find('.v-galleria.v-component').exists()).toBe(true);
        expect(wrapper.findAll('.v-galleria-item-container > .v-galleria-item').length).toBe(1);
        expect(wrapper.findAll('.v-galleria-thumbnail-item-active').length).toBe(2);
    });

    it('should active item change', async () => {
        expect(wrapper.find('.v-galleria-item-container img').attributes().alt).toBe('Description for Image 1');

        wrapper.vm.onActiveItemChange(1);
        await wrapper.setProps({ activeIndex: 1 });

        expect(wrapper.find('.v-galleria-item-container img').attributes().alt).toBe('Description for Image 2');
