...
<script>
import { FilterService } from '@coscom/coscom-ui/api';
...
import SearchIcon from '@coscom/coscom-ui/icons/search';
...
import { ConnectedOverlayScrollHandler, DomHandler, ObjectUtils, UniqueComponentId, ZIndexUtils } from '@coscom/coscom-ui/utils';
import VirtualScroller from '@coscom/coscom-ui/virtualscroller';
...
export default {
    ...
    methods: {
        ...
        getSelectedItemsLabel() {
            let pattern = /{(.*?)}/;
            const selectedItemsLabel = this.selectedItemsLabel || this.$coscom.config.locale.selectionMessage;
    ...
    computed: {
        ...
        filterMessageText() {
            return this.filterMessage || this.$coscom.config.locale.searchMessage || '';
        },
        emptyFilterMessageText() {
            return this.emptyFilterMessage || this.$coscom.config.locale.emptySearchMessage || this.$coscom.config.locale.emptyFilterMessage || '';
        },
        emptyMessageText() {
            return this.emptyMessage || this.$coscom.config.locale.emptyMessage || '';
        },
        ...
        toggleAllAriaLabel() {
            return this.$coscom.config.locale.aria ? this.$coscom.config.locale.aria[this.allSelected ? 'selectAll' : 'unselectAll'] : undefined;
        },
        closeAriaLabel() {
