    ...
    return [
        {
            name: 'Coscom',
            as: 'Coscom',
            from: resolvePath({ name: 'Coscom', as: 'Coscom', from: `coscom/config`, type: 'config' })
        }
    ];
}
...
function registerComponents(resolvePath, options = {}) {
    ...
    return items.map((item) => {
        const _item = { ...item, name: item.name, as: item.name, from: `coscom/${item.name.toLowerCase()}` };
...
function registerServices(resolvePath, registered) {
    ...
    return [...services].map((service) => ({
        name: service,
        as: service,
        from: resolvePath({ name: service, as: service, from: `coscom/${service.toLowerCase()}`, type: 'service' })
    }));
}
    ...
    const styles = [
        {
            name: 'BaseStyle',
            as: 'BaseStyle',
            from: resolvePath({ name: 'BaseStyle', as: 'BaseStyle', from: 'coscom/base/style', type: 'style' })
        }
    ];
    ...
    if (!options?.unstyled) {
        if (Utils.object.isNotEmpty(registered?.components)) {
            styles.push({
                ...
                from: resolvePath({ name: 'BaseComponentStyle', as: 'BaseComponentStyle', from: 'coscom/basecomponent/style', type: 'style' })
            });
        }
        ...
        [registered.components, registered.directives]
            ...
            .forEach((item) =>
                styles.push({
                    name: `${item.as}Style`,
                    as: `${item.as}Style`,
                    from: resolvePath({ name: `${item.as}Style`, as: `${item.as}Style`, from: `coscom/${item.as.toLowerCase()}/style`, type: 'style' })
                })
            );
    }
